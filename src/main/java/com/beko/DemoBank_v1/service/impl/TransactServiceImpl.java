...
public class TransactServiceImpl implements TransactService {
        ...
        try {
            ...
            if (sourceAccountId == targetAccountId) {
                return ResponseEntity.badRequest().body(
                        "Cannot transfer into the same account. Please select the appropriate account to perform the transfer.");
            }
    ...
    private void validatePaymentRequest(PaymentRequest request) {
        ...
        if (StringUtils.isEmpty(beneficiary) || StringUtils.isEmpty(accountNumber) || StringUtils.isEmpty(accountID)
                || StringUtils.isEmpty(paymentAmount)) {
            throw new IllegalArgumentException(
                    "Beneficiary, account number, account paying from, and account payment amount cannot be empty.");
        }
    ...
    private void validateTransferRequest(TransferRequest request) {
        ...
        if (StringUtils.isEmpty(transferFrom) || StringUtils.isEmpty(transferTo)
                || StringUtils.isEmpty(transferAmount)) {
        ...
        if (transferAmountValue == 0) {
            throw new IllegalArgumentException(
                    "Cannot transfer an amount of 0 (zero) value. Please enter a value greater than zero.");
        }
    }
